...
namespace AcademicRecord
{
        ...
        public double this[int CA]
        {
            ...
            set
            {
                ...
                if ((index >= 0) && (index <= next) && (index < (MAX)))
                {
                    ...
                    if (index == next)                          
                    {
                        //If index==next then this is a new result (i.e we are not replacing an existing result
                        //Therefore we move 'next' on one space ready for next new entry                       
                        next++;                                 
                    }
                }
                else
    ...
    class Test
    {
        public static void Main()
        {
            try
            {
                ModuleCAResults results = new ModuleCAResults() { Name = "oosdev", Credits = 10, StudentName = "Jane Doe" };
